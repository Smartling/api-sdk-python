...
class SmartlingApiExample:
    ...
    def testImport(self, name_to_import):
        ...
        resp, code = self.fapi.uploadSourceFile(path, fileUri=path, fileType=self.file_type)
        if 200!=code:
        ...
        resp, code = self.fapi.importFileTranslations(fileUri=path, file=path_to_import, 
                    fileType=self.file_type, localeId=self.MY_LOCALE, 
        ...
        resp, code = self.fapi.getTranslatedFileLastModifiedDateSingleLocale(fileUri=path, localeId=self.MY_LOCALE)
        print(resp, code)
        
        self.printMarker("delete from server goes here")
        #delete test file imported in the beginning of test
        resp, code = self.fapi.deleteUploadedSourceFile(path)
        print(resp, code)
    ...
    def test(self):
        ...
        resp, code = self.fapi.uploadSourceFile(path, fileUri=path, fileType=self.file_type, authorize="true", callbackUrl=CALLBACK_URL, directives=directives)
        print(resp, code)
        if 200!=code:
            raise Exception("failed")
        ...
        resp, code = self.fapi.getFileTranslationStatusAllLocales(path)
        print(resp, code)

        self.printMarker("file from server goes here")
        resp, code = self.fapi.downloadTranslatedFileSingleLocale(self.MY_LOCALE, path)
        print(resp, code)

        self.printMarker("renaming file")
        resp, code = self.fapi.renameUploadedSourceFile(path, self.new_name)
        print(resp, code)

        self.printMarker("delete from server goes here")
        resp, code = self.fapi.deleteUploadedSourceFile(self.new_name)
        print(resp, code)
    ...
    def delAll(self):
        self.printMarker("files list to delete")
        resp, code = self.fapi.getRecentlyUploadedSourceFilesList()
...
def importTest():
    ...
    example.testImport(FILE_NAME_TO_IMPORT)
